...
describe('PromptProcessingService', () => {
  ...
  let factory: Partial<FlashcardStrategyFactory>;

  beforeEach(async () => {
    userRepo = {
      findUserApiKey: jest.fn(),
    };

    factory = {
      getStrategyForApiKey: jest.fn(),
    };
  ...
  it('should throw UnauthorizedException if user has no API key', async () => {
    (userRepo.findUserApiKey as jest.Mock).mockResolvedValue({});
    await expect(
  ...
  it('should call strategy and return result', async () => {
    const mockStrategy = {
      generateFlashcards: jest
        .fn()
        .mockResolvedValue({ parsedFlashcards: [1, 2], totalCards: 2 }),
    };
    (userRepo.findUserApiKey as jest.Mock).mockResolvedValue({
      openAiApiKey: 'key123',
    });
    (factory.getStrategyForApiKey as jest.Mock).mockReturnValue(mockStrategy);

    const result = await service.processPrompt(
      { text: 'hi', deckName: 'Deck 1' },
      'user1'
    );

    expect(mockStrategy.generateFlashcards).toHaveBeenCalledWith(
      'hi',
      'key123'
    );
